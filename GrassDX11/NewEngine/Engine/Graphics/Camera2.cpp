...
void Camera2::updateDirectionVectors() {
...
Camera2::Camera2(float fov, float aspectRatio, float nearZ, float farZ, XMVECTOR startPos)
	: pitch(0.0f), yaw(0.0f) {
...
void Camera2::setPosition(XMVECTOR newPos) {
	this->position = newPos;
}

void Camera2::move(XMVECTOR dirV) {
...
void Camera2::rotate(XMVECTOR dirV) {
	XMFLOAT4 dir;
	XMStoreFloat4(&dir, dirV);
...
const XMMATRIX& Camera2::getViewMatrix() {
	this->updateViewMatrix();

	return this->viewMatrix;
}

const XMFLOAT3 Camera2::getPosition() const {
	XMFLOAT3 tempFloat3;
